...
public:
    ...
    piecewise_function* redefine_on_interval(double start, double end, double k, double m) override {//переопр
        ...
        for (int i = 0; i < segments_->get_length(); ++i) {
            ...
            if (seg.end_x <= start || seg.start_x >= end) {
                new_segs = new_segs->append(seg); //копируем без изм
            }
            else { //разбиваем на части
    ...
    piecewise_function* add_segment(double start, double end, double k, double m) override {//в конец
        segments_ = segments_->append(segment(start, end, k, m));
        return this;
    }

    piecewise_function* remove_segment(int index) override { //по индексу
    ...
    piecewise_function* concat(const piecewise_function& other) override { //сцеп с др ф-ией
